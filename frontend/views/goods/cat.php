        ...
        if(\Yii::$app->user->isGuest) {
            ...
            foreach ($goods_models as $model):?>
                <tr data-goods_id="<?= $model['id'] ?>">
                    ...
                    <td class="col4">
                        <a href="javascript:;" class="reduce_num"></a>
                        <input type="text" name="amount" value="<?=$amount[$model->id]?$amount[$model->id]:null?>" class="amount"/>
                        <a href="javascript:;" class="add_num"></a>
                    </td>
                    <td class="col5">￥<span><?= $amount[$model->id]?$model['shop_price']*$amount[$model->id].'.00':null ?></span></td>
                    <td class="col6"><?=\yii\helpers\Html::a('删除',['goods/update','id'=>$model['id']])?></td>
                </tr>
                <?php
            endforeach;
        }
